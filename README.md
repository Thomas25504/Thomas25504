# Hi, I’m Tom Davidson 👋

I'm a 3rd-year Software Engineering student passionate about building clean, efficient applications.  
Right now, I'm focused on **mobile development with C# and .NET MAUI** and levelling up my **AWS cloud skills**.

---

## 🚀 What I’m Working On
- 📱 **Mobile Computing** using **C#** & **.NET MAUI**
- ☁️ Learning **AWS** (EC2 fundamentals) and **Google Cloud** (Compute Engine fundamentals)
- 🧪 Strengthening architecture, testing, and deployment workflows

---

## 🧰 Tech Stack
### **Core Focus**
- **C#**, **.NET**, **.NET MAUI**, **AWS (learning)**  
- **SQL**, **REST APIs**, **Clean Architecture**

### **Other Experience**
- **Node.js**, **Express**, **React**, **Vue.js**, **TypeScript**
- **Python**, **Java**
- **Git**, **GitHub**, **SQLite**, **TailwindCSS**

![Node.js](https://img.shields.io/badge/-Node.js-773ca6?style=for-the-badge&logo=node.js&logoColor=white)
![Git](https://img.shields.io/badge/-Git-A42095?style=for-the-badge&logo=git&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-CA3973?style=for-the-badge&logo=javascript&logoColor=white)
![TypeScript](https://img.shields.io/badge/-TypeScript-F66942?style=for-the-badge&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/-React-FEA545?style=for-the-badge&logo=react&logoColor=white)
![Next.js](https://img.shields.io/badge/-Next.js-773ca6?style=for-the-badge&logo=nextdotjs&logoColor=white)
![Vue.js](https://img.shields.io/badge/-Vue.js-A42095?style=for-the-badge&logo=vue.js&logoColor=white)
![Express.js](https://img.shields.io/badge/-Express.js-F66942?style=for-the-badge&logo=express&logoColor=white)
![Python](https://img.shields.io/badge/-Python-FEA545?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-CA3973?style=for-the-badge&logo=openjdk&logoColor=white)
![SQLite](https://img.shields.io/badge/-SQLite-A42095?style=for-the-badge&logo=sqlite&logoColor=white)
![TailwindCSS](https://img.shields.io/badge/-Tailwind%20CSS-773ca6?style=for-the-badge&logo=tailwindcss&logoColor=white)
---

## 📦 Featured Projects

### 🔹 Chaos Theory – Lorenz Attractor (Python)  
A visual simulation of the Lorenz Attractor.  
🔗 https://github.com/Thomas25504/Chaos-Theory-Lorenz-Attractor

### 🔹 Event Booking App (Full-Stack Vue.js)  
A complete event creation & registration platform.  
🔗 https://github.com/Thomas25504/fsd_eventitude

---

## 📫 Connect With Me
- **LinkedIn:** https://www.linkedin.com/in/tomdavidson2004  
- **Email:** tldavidson2004@gmail.com  

---

## ⚡ Fun Facts
- Consistent gym training taught me discipline that I carry into coding  
- I love experimenting with new frameworks just to see what's possible  
